...
func _ready():
	...
	print.debug("Socket A pos: ", socket_out.global_position, " forward: ", -socket_out.global_transform.basis.z)

	_align_room_to_socket(room_b, socket_out)

	var entry = room_b.sockets[0]  # celui qui vient d'être utilisé
	print.debug("Socket B (après alignement) pos: ", entry.global_position, " forward: ", -entry.global_transform.basis.z)
	"""
